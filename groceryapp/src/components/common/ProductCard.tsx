"use client";

import { ProductTypesProps } from "@/Types/Data";
import Image from "next/image";
import React, { useState } from "react";
import { CarrotIcon, ShoppingBag } from "lucide-react";
import { useRouter } from "next/navigation";
import { assets } from "../../../public/assets/assets";

const ProductCard = ({ product }: { product: ProductTypesProps }) => {
  const [quantity, setQuantity] = useState(0);


  const router = useRouter()
  
  return (
   <div
        onClick={() => {
          router.push(`/products/${product._id}`);
          window.scrollTo({ left: 0, top: 0, behavior: "smooth" });
        }}
        className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full cursor-pointer"
      >
        <div className="group flex items-center justify-center px-2">
          <Image
            className="group-hover:scale-105 transition max-w-26 md:max-w-36"
            src={product.image[0]}
            alt={product.name}
            loading="lazy"
          />
        </div>
        <div className="text-gray-500/60 text-sm">
          <p>{product.category}</p>
          <p className="text-gray-700 font-medium text-lg truncate w-full">
            {product.name}
          </p>
          <div className="flex items-center gap-0.5">
            {Array(5).fill("").map((_, i) => (
              <Image
                key={i}
                className="md:w-3.5 w-3"
                src={i < 4 ? assets.star_icon : assets.star_dull_icon}
                alt=""
              />
            ))}
            <p>({4})</p>
          </div>
          <div className="flex items-end justify-between mt-3">
            <p className="md:text-xl text-base font-medium text-btn">
              {"currency"}
              {product.offerPrice}{" "}
              <span className="text-gray-500/60 md:text-sm text-xs line-through">
                {"currency"}
                {product.price}
              </span>
            </p>
            <div className="text-btn" onClick={(e) => e.stopPropagation()}>
              {quantity === 0 ? (
                <button 
                  className="flex items-center justify-center gap-1 bg-greenLight/10 border hover:scale-95 transition-all duration-200 border-btn md:w-[80px] w-[64px] h-[34px] rounded text-greenLight font-medium"
                >
                  <ShoppingBag/>
                  Add
                </button>
              ) : (
                <div className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-btn/25 rounded select-none">
                  <button 
                    className="cursor-pointer text-md px-2 h-full flex items-center justify-center"
                  >
                    -
                  </button>
                  <span className="w-5 text-center">{quantity}</span>
                  <button 
                    className="cursor-pointer text-md px-2 h-full flex items-center justify-center"
                  >
                    +
                  </button>
                </div>
              )}
            </div>
          </div>
        </div>
      </div>
  );
};

export default ProductCard;
